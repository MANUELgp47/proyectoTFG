...
export const finalizarActividad = async (req: Request, res: Response) => {
    ...
    try {
        ...
        //si es creador o admin puede finalizar la actividad
        const esCreador: boolean = await ActividadService.ActividadService.esCreadorActividad(idActividad, req.userId!) || await UsuarioService.getRolPorIdUsuario(Number(req.userId)) === 'admin';
        const esAdminActividad: boolean = await ActividadService.ActividadService.esAdminActividad(idActividad, req.userId!);
...
export const deleteActividad = async (req: Request, res: Response) => {
    ...
    try {
        ...
        //el rol es de mod o admin solamente
        const rol = await UsuarioService.getRolPorIdUsuario(Number(req.userId));
...
//añade o elimina admins del campo actividad.admins. Solo si es creado o un admin
export const editAdmins = async (req: Request, res: Response) => {
    const idUsuario = req.userId;
    const idActividad = Number(req.params.idActividad);
    const {idAdmin, action} = req.body;

    // Validar que el usuario es el creador o un admin
    const esCreador = await ActividadService.ActividadService.esCreadorActividad(idActividad, Number(idUsuario));
    const esAdmin = await ActividadService.ActividadService.esAdminActividad(idActividad, Number(idUsuario));
    if (!esCreador && !esAdmin) {
        return res.status(403).json({message: 'No tienes permiso para editar los admins de esta actividad'});
    }

    //idAdmin es un participante de la actividad
    const esParticipante = await ActividadService.ActividadService.esParticipante(idActividad, idAdmin);
    if (!esParticipante) {
        return res.status(400).json({message: 'El usuario que quieres añadir como admin no es participante de la actividad'});
    }

    try {
        if (action === 'add') {
            await ActividadModel.addAdminActividad(idActividad, idAdmin);
            res.json({message: 'Admin añadido correctamente'});
        } else if (action === 'remove') {
            await ActividadModel.removeAdminActividad(idActividad, idAdmin);
...
export const editExpulsados = async (req: Request, res: Response) => {
    const idUsuario = req.userId;
    const idActividad = Number(req.params.idActividad);
    const {idExpulsado, action} = req.body;

    // Validar que el usuario es el creador o un admin
    const esCreador = await ActividadService.ActividadService.esCreadorActividad(idActividad, Number(idUsuario));
    const esAdmin = await ActividadService.ActividadService.esAdminActividad(idActividad, Number(idExpulsado));
    if (!esCreador && !esAdmin) {
        return res.status(403).json({message: 'No tienes permiso para editar los expulsados de esta actividad'});
    }

    //No puedes tener acciones sobre ti mismo
    if (idExpulsado === idUsuario) {
        return res.status(400).json({message: 'No puedes expulsarte o reincorporarte a ti mismo'});
    }
    ...
    try {
        if (action === 'add') {
            ...
            //Elimina la participacion del usuario expulsado
            await ParticipacionModel.eliminarParticipacion(idExpulsado, idActividad);


            res.json({message: 'Usuario expulsado correctamente'});
        } else if (action === 'remove') {
            await ActividadModel.removeExpulsadoActividad(idActividad, idExpulsado);
            //crea la participacion del usuario reincorporado
            await ParticipacionModel.crearParticipacion({
                idUsuario: idExpulsado,
                idActividad: idActividad,
                esCreador: false,
                aceptada: true,
            });

            res.json({message: 'Usuario reincorporado correctamente'});
        } else {
            res.status(400).json({message: 'Acción inválida'});
        }
