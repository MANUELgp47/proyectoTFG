...
export const getAmistadPorUsuarios = async (idUsuario1: number, idUsuario2: number): Promise<Amistad | null> => {
    const result = await pool.query("SELECT * FROM amistad WHERE id_usuario_1 = $1 AND id_usuario_2 = $2", [idUsuario1, idUsuario2]);
...
export const getAmistadesPorUsuario = async (idUsuario: number): Promise<Amistad[]> => {
    const result = await pool.query("SELECT * FROM amistad WHERE id_usuario_1 = $1 OR id_usuario_2 = $1", [idUsuario]);
    return result.rows.map(mapearAmistad);
};

export const crearAmistad = async (amistad: CrearAmistad): Promise<Amistad> => {
    const {
        idUsuario1,
        idUsuario2,
    } = amistad;
...
export const eliminarAmistad = async (idUsuario1: number, idUsuario2: number): Promise<boolean> => {
    const result = await pool.query("DELETE FROM amistad WHERE id_usuario_1 = $1 AND id_usuario_2 = $2", [idUsuario1, idUsuario2]);
