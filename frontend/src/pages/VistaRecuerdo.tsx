    ...
    return (
        <div className="min-h-screen bg-[#F8F9FB]">
            ...
            RECUERDO PUBLICADO
                        ...
                        <h1 className="mt-4 text-4xl sm:text-5xl font-extrabold text-secondary tracking-tight leading-tight" style={{fontFamily: "'Manrope', sans-serif"}}>
                            {recuerdo.titulo}
                        </h1>
                    ...
                    {hayImagenes && (
                        <aside>
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="text-xl font-extrabold text-secondary" style={{fontFamily: "'Manrope', sans-serif"}}>
                                    Galería de momentos
                                </h2>
                                    ...
                                    {recuerdo.imagenes.length > 1 && (
                                        <div className="grid grid-cols-2 gap-3">
                                            {recuerdo.imagenes.slice(1, 5).map((url, i, arr) => {
                                                ...
                                                return (
                                                    <div key={i} className="relative aspect-square bg-black rounded-2xl overflow-hidden">
                                                        <img
                                                            src={url}
                                                            alt={`Foto ${i + 2}`}
                                                            className="absolute inset-0 w-full h-full object-cover"
                                ...
                                >
                                    <div className="w-6 h-6 rounded-full bg-secondary flex items-center justify-center overflow-hidden">
                                        {p.imagen ? (
                                            ...
                                            <span className="text-white text-xs font-bold">{p.nombreUsuario.charAt(0).toUpperCase()}</span>
                                        )}
                                    </div>
                                    {p.nombreUsuario}
                                </Link>
                ...
                <section className="mt-12">
                    ...
                    <form onSubmit={(e) => {
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <textarea name="texto" required placeholder="Escribe un comentario..." className="flex-1 min-h-[80px] resize-y bg-neutral-light rounded-xl px-4 py-3 text-sm text-secondary placeholder-neutral outline-none focus:ring-2 focus:ring-primary-100" />
                        </div>
                        <div className="mt-3 flex justify-end">
                            <button type="submit" className="px-6 py-2.5 rounded-full bg-primary text-white font-semibold text-sm hover:bg-primary-600 transition">
                                Publicar
                            </button>
                        </div>
                    </form>
                                ...
                                <div key={c.idComentario} className="flex items-start gap-3">
                                    <div className="w-9 h-9 rounded-full bg-secondary overflow-hidden flex items-center justify-center text-white text-xs font-bold shrink-0">
                                        {mapaUsuarioMinimo[c.idUsuario]?.imagen ? (
                                            <img src={mapaUsuarioMinimo[c.idUsuario]!.imagen!} alt={mapaUsuarioMinimo[c.idUsuario]?.nombreUsuario ?? `Usuario ${c.idUsuario}`} className="w-full h-full object-cover" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
                                        ) : (
      ...
      {(mapaUsuarioMinimo[c.idUsuario]?.nombreUsuario ?? c.idUsuario.toString()).charAt(0).toUpperCase()}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="bg-white rounded-2xl px-4 py-3 shadow-sm">
                                            <div className="flex items-center justify-between gap-4">
                                                <Link to={`/usuario/${c.idUsuario}`} className="text-sm font-bold text-secondary hover:underline">
        ...
        {c.fechaCreacion ? new Date(c.fechaCreacion).toLocaleDateString() : "Hace un rato"}
                                        ...
                                        <div className="mt-2 flex items-center gap-4 pl-2">
                                            <button onClick={() => handleLikeComentario(c.idComentario)} disabled={heDadoLikeComentarios[c.idComentario] === true} className="inline-flex items-center gap-1.5 text-xs font-semibold text-neutral hover:text-red-500 transition">
                                                <Heart className={`w-3.5 h-3.5 ${heDadoLikeComentarios[c.idComentario] ? "fill-red-500 text-red-500" : ""}`} />
                                                {likesComentarios[c.idComentario] ?? 0}
                                            </button>

                                            {(rol === "admin" || rol === "mod") && (
                                                <button onClick={() => handleEliminaComentario(c.idComentario)} className="inline-flex items-center gap-1 text-xs font-semibold text-red-500 hover:text-red-600 transition">
                                                    <Trash2 className="w-3.5 h-3.5"/>
                                                    Eliminar
                                                </button>
                                            )}
